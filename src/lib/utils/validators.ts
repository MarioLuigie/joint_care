import { RegistrationFormErrors, RegistrationFormData } from '@/lib/types'
...
const V = {
	...
	lettersize: (data: string, msg: string) =>
		/[a-z]/.test(data) && /[A-Z]/.test(data) ? '' : msg,
	specialChar: (data: string, msg: string) =>
		/[^A-Za-z0-9]/.test(data) ? '' : msg,
	email: (data: string, msg: string) =>
		/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(data) ? '' : msg,
	minLength: (num: number, data: string, msg: string) =>
		data.length >= num ? '' : msg,
	maxLength: (num: number, data: string, msg: string) =>
		data.length <= num ? '' : msg,
	match: (data1: string, data2: string, msg: string) =>
		data1 === data2 ? '' : msg,
